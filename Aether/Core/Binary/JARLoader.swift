...
nonisolated final class JARLoader: BinaryLoaderProtocol {
    ...
    func load(data: Data) throws -> BinaryFile {
        ...
        if data[0] == 0x50 && data[1] == 0x4B {
			return try loadJAR(data: data)
        } else {
			return try loadClassFile(data: data)
        }
    }

    // MARK: - Loading

	func loadJAR(data: Data) throws -> BinaryFile {
        ...
        if let manifestData = try? archive.fileContents(at: "META-INF/MANIFEST.MF"),
		...
		for entry in try archive.entries {
			guard entry.path.hasSuffix(".class") else { continue }
			...
			let className = entry.path.replacingOccurrences(of: "/", with: ".").replacingOccurrences(of: ".class", with: "")
			...
			baseAddress += UInt64(data.count) + 0x1000
		}
	...
	func loadClassFile(data: Data) throws -> BinaryFile {
        let javaClass = try parseClassFile(data: data)
